...
namespace BoidsSimulationOnGPU {
    public class GPUBoids : MonoBehaviour {
        ...
        public int MaxObjectNum {
            get => maxObjectNum;
            set {
                maxObjectNum = value - value % SIMULATION_BLOCK_SIZE;
            }
        }
        #endregion
        ...
        void Start() {
            // バッファを初期化
            InitBuffer();
            chaser.SetBounds(WallCenter, WallSize);
        }

        void Update() {
            maxObjectNum -= maxObjectNum % SIMULATION_BLOCK_SIZE;
            // シミュレーション
            Simulation();
        }
        ...
        void InitBuffer() {
            ...
            for (var i = 0; i < maxObjectNum; i++) {
                ...
                boidDataArr[i].Position = Vector3.Scale(Random.insideUnitSphere, GetSimulationAreaSize() * 0.5f);
        ...
        void Simulation() {
            ...
            cs.SetVector("_ChaserPos", chaser.transform.position);
